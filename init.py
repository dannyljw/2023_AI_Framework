...
def xavier_uniform_(tensor: Tensor, gain: float = 1.) -> Tensor:
    ...
    # print("xavier_uniform_gain:",gain)
...
def m2_calculate_correct_fan(tensor, mode):
    mode = mode.lower()
    valid_modes = ['fan_in', 'fan_out']
    if mode not in valid_modes:
        raise ValueError("Mode {} not supported, please use one of {}".format(mode, valid_modes))
...
def m1_kaiming_uniform_(
    ...
    gain = calculate_gain(nonlinearity, a)
    #m2
    std = gain / math.sqrt(math.sqrt(fan))
    # m3 
    #std = gain / {fan * math.sqrt(fan)}
    bound = math.sqrt(3.0) * std  # Calculate uniform bounds from standard deviation
    with torch.no_grad():
        return tensor.uniform_(-bound, bound)

def m2_kaiming_uniform_(
    tensor: Tensor, a: float = 0, mode: str = 'fan_in', nonlinearity: str = 'leaky_relu'
):
    r"""Fills the input `Tensor` with values according to the method
    described in `Delving deep into rectifiers: Surpassing human-level
    ...
    if 0 in tensor.shape:
        ...
        return tensor
    #m2
    fan = m2_calculate_correct_fan(tensor, mode)
    gain = calculate_gain(nonlinearity, a)
    std = gain / math.sqrt(math.sqrt(fan))
    bound = math.sqrt(3.0) * std  # Calculate uniform bounds from standard deviation
    with torch.no_grad():
